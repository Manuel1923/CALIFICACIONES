...
const ListaCiclos = () => {
  ...
  const validarCiclo = () => {
    ...
    const nombreCiclo = typeof nuevoCiclo === 'string' ? nuevoCiclo.trim() : nuevoCiclo.Nombre_ciclo.trim();
  
    // Comprobación de vacío
    if (!nombreCiclo || nombreCiclo === '') {
  ...
  const validarCicloUpdate = () => {
    // Eliminar espacios al inicio y al final del nombre al actualizar
    const nombreCiclo = cicloToUpdate.Nombre_ciclo.trim();
    ...
    const cicloExistente = ciclos.some(
      (ciclo) =>
        ciclo.Nombre_ciclo.toLowerCase() === nombreCiclo.toLowerCase() &&
