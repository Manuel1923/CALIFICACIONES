...
const ListaGrados = () => {
  ...
  const validarGrado = () => {
    ...
    const nombreGrado = typeof nuevoGrado === 'string' ? nuevoGrado.trim() : nuevoGrado.Nombre_grado.trim();
  
    // Validar que el campo de nombre no esté vacío
    if (!nombreGrado || nombreGrado === '') {
      Swal.fire('Error', 'El campo "Nombre Grado" no puede estar vacío', 'error');
      return false;
    }
  
    // Validar que el grado no exista en la lista
  ...
  const validarGradoUpdate = () => {
    // Eliminar espacios al inicio y al final
    const nombreGrado = gradoToUpdate.Nombre_grado.trim();
    ...
    // Verificar si el grado ya existe (excluyendo el que se está editando)
    const gradoExistente = grados.some(
      (grado) =>
        grado.Nombre_grado.toLowerCase() === nombreGrado.toLowerCase() &&
