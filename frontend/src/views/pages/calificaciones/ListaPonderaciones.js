...
const ListaPonderaciones = () => {
  ...
  const validarPonderacion = () => {
    // Aplicar trim() para eliminar espacios al inicio y al final
    const nombrePonderacion = typeof nuevaPonderacion === 'string' ? nuevaPonderacion.trim() : nuevaPonderacion.Descripcion_ponderacion.trim();
  
    // Comprobación de vacío
    if (!nombrePonderacion || nombrePonderacion === '') {
      Swal.fire('Error', 'El campo "Descripción de la Ponderación" no puede estar vacío', 'error');
      return false;
    }
  
    // Verificar si la ponderación ya existe
  ...
  const validarPonderacionUpdate = () => {
    // Aplicar trim() para eliminar espacios al inicio y al final
    const nombrePonderacion = ponderacionToUpdate.Descripcion_ponderacion.trim();
    ...
    // Verificar si la ponderación ya existe (excluyendo la actual)
    const ponderacionExistente = Ponderaciones.some(
      (ponderacion) =>
        ponderacion.Descripcion_ponderacion.toLowerCase() === nombrePonderacion.toLowerCase() &&
  ...
  const handleInputChange = (e, setFunction) => {
    ...
    const regex = /^[A-Z-1-9Ñ\s]*$/; // Solo letras y espacios
  ...
  <CRow className="align-items-center mb-5">
   ...
   <CTableHead>
       <CTableRow>
           ...
           <CTableHeaderCell style={{ width: '300px', whiteSpace: 'normal', wordBreak: 'break-word' }}>
               Descripcion de la Ponderacion
           </CTableHeaderCell>
   ...
   <CTableBody>
       {currentRecords.map((ponderacion) => (
           <CTableRow key={ponderacion.Cod_ponderacion}>
               <CTableDataCell>{ponderacion.originalIndex}</CTableDataCell>
               <CTableDataCell style={{ whiteSpace: 'normal', wordBreak: 'break-word' }}>
        ...
        <CModalBody>
          <CForm>
            <CInputGroup className="mb-3">
              ...
              <CFormTextarea 
               ref={inputRef}
              rows={3} // Ajusta el número de filas para mayor altura
              maxLength={100}
              onPaste={disableCopyPaste}
              onCopy={disableCopyPaste}
