...
const ListaParciales = () => {
  ...
  const resetParcial= () => setNuevoParcial('');
  const resetParcialtoUpdate = () => setParcialesToUpdate('');
  useEffect(() => {
    fetchParciales();
  }, []);
...
const validateParcial = () => {
  ...
  const parcialExistente = Parciales.some(
    (parcial) => parcial.Nombre_parcial.toLowerCase() === nuevoParcial.Nombre_parcial.toLowerCase()
  );

  if (parcialExistente) {
    Swal.fire('Error', 'Ya existe un parcial con este nombre', 'error');
    return false;
  }
...
const handleInputChange = (e, setFunction) => {
  ...
  const regex = /^[A-Za-zñÑ\s]*$/; // Solo letras y espacios
  ...
  const handleUpdateParcial = async () => {
    ...
    try {
      ...
      if (!parcialToUpdate.Nota_recuperacion) {
        Swal.fire('Error', 'La nota de recuperacion es obligatoria', 'error');
        return false;
      }
    } catch (error) {
      Swal.fire('Error', 'Hubo un problema al actualizar el parcial', 'error');
